...
class GPT2Attention(nn.Module):
    def __init__(self, config, kqv=None, is_cross_attention=False, layer_idx=None):
        super().__init__()
        ...
        if kqv is None: 
            ...
            if self.is_cross_attention:
                self.c_attn = hydra.utils.instantiate(kqv, self.hidden_size, 2 * self.embed_dim,
                                               _recursive_=False)
                self.q_attn = hydra.utils.instantiate(kqv, self.hidden_size, self.embed_dim,
                                               _recursive_=False)
            else:
                self.c_attn = hydra.utils.instantiate(kqv, self.hidden_size, 3 * self.embed_dim,
                                               _recursive_=False)
            self.c_proj = hydra.utils.instantiate(kqv, self.embed_dim, self.hidden_size,
                                               _recursive_=False)
...
class GPT2Block(nn.Module):
    def __init__(self, config, layer_idx=None, mlp_cfg=None, kqv=None):
        super().__init__()
        hidden_size = config.hidden_size
        inner_dim = config.n_inner if config.n_inner is not None else 4 * hidden_size

        self.ln_1 = nn.LayerNorm(hidden_size, eps=config.layer_norm_epsilon)
        self.attn = GPT2Attention(config, kqv=kqv, layer_idx=layer_idx)
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def __init__(self, config, mlp_cfg=None, kqv=None):
        ...
        self.h = nn.ModuleList([GPT2Block(config, layer_idx=i, mlp_cfg=mlp_cfg, kqv=kqv)
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def __init__(self, config, mlp_cfg=None, lm_head=None, kqv=None):
        
        super().__init__(config)
        
        self.transformer = GPT2Model(config, mlp_cfg=mlp_cfg, kqv=kqv)
